...
#### TODO: Refactor after converting to pytorch3d
...
def close_holes(vertices, faces):
    ...
    #### Generate patches
    center_all = torch.mean(vertices, axis=0)
    ...
    for hole in holes:
        ...
        vertice_new = torch.mean(vertices_oi, axis=0)
    ...
    return torch.Tensor(_vertices), torch.Tensor(_faces).type(torch.long)
